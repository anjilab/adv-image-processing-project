...
import probe, tti, gen_cub_synthetic, hyperopt
...
def train_X_to_C(args):
    ...
    for param in model.parameters():
        param.requires_grad = not args.freeze  # If freeze=True, this will stop training on base layers

    # Get the number of trainable parameters
    num_trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

    print(f"Number of trainable parameters: {num_trainable_params}") # 24658880
    train(model, args)
